import { Button } from "@/components/ui/button";
import { Instagram, Music, Users, Star } from "lucide-react";

const Artists = () => {
  const featuredArtists = [
    {
      name: "Vincxnt",
      genre: "Trap/Drill",
      bio: "Pioneiro do drill nacional com flow único e letras que retratam a realidade da periferia com poetismo urbano.",
      image: "https://images.unsplash.com/photo-1520813792240-56fc4a3765a7?w=400&h=400&fit=crop&crop=faces",
      stats: { tracks: 15, streams: "2.5M", followers: "45K" },
      instagram: "@vincxnt_oficial",
      featured: true
    },
    {
      name: "Eiijay",
      genre: "Trap Melódico",
      bio: "Voz marcante e melodias viciantes que mesclam trap com influências do R&B contemporâneo.",
      image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&h=400&fit=crop&crop=faces",
      stats: { tracks: 12, streams: "1.8M", followers: "32K" },
      instagram: "@eiijay_music",
      featured: true
    },
    {
      name: "Gabola",
      genre: "Rap Consciente",
      bio: "Letras profundas sobre questões sociais e crescimento pessoal, com uma pegada old school renovada.",
      image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&h=400&fit=crop&crop=faces",
      stats: { tracks: 8, streams: "950K", followers: "28K" },
      instagram: "@gabola_rap",
      featured: true
    }
  ];

  const emergingArtists = [
    {
      name: "MC Drift",
      genre: "Phonk Brasileiro",
      image: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=300&h=300&fit=crop&crop=faces",
      tracks: 5
    },
    {
      name: "Lila Trap",
      genre: "Trap Feminino",
      image: "https://images.unsplash.com/photo-1494790108755-2616b612b788?w=300&h=300&fit=crop&crop=faces",
      tracks: 7
    },
    {
      name: "Flow Pesado",
      genre: "Drill Nacional",
      image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=300&h=300&fit=crop&crop=faces",
      tracks: 4
    },
    {
      name: "Rima Urbana",
      genre: "Rap Experimental",
      image: "https://images.unsplash.com/photo-1463453091185-61582044d556?w=300&h=300&fit=crop&crop=faces",
      tracks: 6
    }
  ];

  return (
    <section id="artistas" className="py-20 bg-gradient-dark">
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          {/* Section Title */}
          <div className="text-center mb-16">
            <h2 className="text-5xl md:text-6xl font-roc font-black text-gradient-neon mb-6">
              NOSSOS ARTISTAS
            </h2>
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
              Talentos únicos que confiam na Golden Key para levar sua música ao próximo nível. 
              Conheça quem faz parte da nossa família artística.
            </p>
          </div>

          {/* Featured Artists */}
          <div className="mb-16">
            <h3 className="text-3xl font-roc font-bold text-gradient-golden mb-8 text-center">
              Artistas em Destaque
            </h3>
            
            <div className="grid md:grid-cols-3 gap-8">
              {featuredArtists.map((artist, index) => (
                <div 
                  key={index}
                  className="urban-card group hover:animate-pulse-glow cursor-pointer"
                >
                  {/* Artist Image */}
                  <div className="relative mb-6 overflow-hidden rounded-lg">
                    <img 
                      src={artist.image} 
                      alt={artist.name}
                      className="w-full aspect-square object-cover group-hover:scale-110 transition-transform duration-500"
                    />
                    <div className="absolute inset-0 bg-gradient-chaos opacity-0 group-hover:opacity-30 transition-opacity duration-300"></div>
                    <div className="absolute top-4 right-4 bg-primary text-primary-foreground px-3 py-1 rounded-full text-xs font-bold">
                      <Star className="w-3 h-3 inline mr-1" />
                      DESTAQUE
                    </div>
                  </div>

                  {/* Artist Info */}
                  <h4 className="text-2xl font-roc font-black text-gradient-golden mb-2 group-hover:text-gradient-electric transition-all duration-300">
                    {artist.name}
                  </h4>
                  <p className="text-secondary font-semibold mb-3">
                    {artist.genre}
                  </p>
                  <p className="text-muted-foreground text-sm mb-4 leading-relaxed">
                    {artist.bio}
                  </p>

                  {/* Stats */}
                  <div className="grid grid-cols-3 gap-2 mb-4 text-center">
                    <div>
                      <div className="text-lg font-roc font-bold text-primary">
                        {artist.stats.tracks}
                      </div>
                      <div className="text-xs text-muted-foreground">Faixas</div>
                    </div>
                    <div>
                      <div className="text-lg font-roc font-bold text-secondary">
                        {artist.stats.streams}
                      </div>
                      <div className="text-xs text-muted-foreground">Streams</div>
                    </div>
                    <div>
                      <div className="text-lg font-roc font-bold text-accent">
                        {artist.stats.followers}
                      </div>
                      <div className="text-xs text-muted-foreground">Seguidores</div>
                    </div>
                  </div>

                  {/* Social Link */}
                  <Button variant="outline" className="w-full neon-border group-hover:btn-electric transition-all duration-300">
                    <Instagram className="mr-2 h-4 w-4" />
                    {artist.instagram}
                  </Button>
                </div>
              ))}
            </div>
          </div>

          {/* Emerging Artists */}
          <div className="mb-16">
            <h3 className="text-3xl font-roc font-bold text-gradient-electric mb-8 text-center">
              Novos Talentos
            </h3>
            
            <div className="grid md:grid-cols-4 gap-6">
              {emergingArtists.map((artist, index) => (
                <div 
                  key={index}
                  className="urban-card text-center group hover:scale-105 transition-all duration-300 cursor-pointer"
                >
                  <div className="relative mb-4 overflow-hidden rounded-full mx-auto w-24 h-24">
                    <img 
                      src={artist.image} 
                      alt={artist.name}
                      className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
                    />
                  </div>
                  <h4 className="font-roc font-bold text-foreground mb-1 group-hover:text-gradient-golden transition-all duration-300">
                    {artist.name}
                  </h4>
                  <p className="text-xs text-muted-foreground mb-2">
                    {artist.genre}
                  </p>
                  <div className="text-xs text-primary font-semibold">
                    {artist.tracks} faixas
                  </div>
                </div>
              ))}
            </div>
          </div>

          {/* Join Us CTA */}
          <div className="text-center">
            <div className="urban-card p-8 max-w-4xl mx-auto">
              <h3 className="text-3xl font-roc font-bold text-gradient-chaos mb-4">
                Quer fazer parte da família Golden Key?
              </h3>
              <p className="text-xl text-muted-foreground mb-6">
                Estamos sempre em busca de novos talentos. Se você tem o flow, 
                nós temos a estrutura para te levar longe.
              </p>
              <div className="flex flex-col md:flex-row gap-4 justify-center">
                <Button className="btn-golden text-lg py-4 px-8">
                  <Users className="mr-2 h-5 w-5" />
                  Quero fazer parte da GK
                </Button>
                <Button variant="outline" className="neon-border text-lg py-4 px-8">
                  <Music className="mr-2 h-5 w-5" />
                  Enviar Demo
                </Button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Artists;